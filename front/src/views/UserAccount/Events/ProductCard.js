...
const ProductCard = ({
  ...
  const handleSubmit = (e) => {
    ...
    try {
      ...
      fetch(process.env.REACT_APP_URL + `user/event/${info.id}`, {
        method: 'post',
        body: formData,
        headers: {
  ...
  const deleteItem = (e, id) => {
    ...
    if (window.confirm('Are you sure you want to delete it?'))
      try {
        fetch(process.env.REACT_APP_URL + `user/event/${id}`, {
          method: 'delete',
          // body: formData,
          headers: {
            Accept: 'application/json',
  ...
  const deleteImage = (e, id) => {
    ...
    if (window.confirm('Are you sure you want to delete it?'))
      try {
        fetch(process.env.REACT_APP_URL + `user/image/${id}`, {
          method: 'delete',
          // body: formData,
          headers: {
            Accept: 'application/json',
  ...
  const deleteHobby = (e, idEvent, idHobby) => {
    e.preventDefault();
    if (window.confirm('Are you sure you want to Remove it?'))
      try {
        fetch(process.env.REACT_APP_URL + `user/hobby/${idEvent}/${idHobby}`, {
          method: 'delete',
          // body: formData,
          headers: {
            Accept: 'application/json',
            ...
            {event &&
              ...
              event.hobbies.map((hobby) => (
                <Grid item xs>
                  <Chip
                    label={hobby.name}
                    onDelete={(e) => deleteHobby(e, event.id, hobby.id)}
                  />
                </Grid>
              ))}
          </Grid>
