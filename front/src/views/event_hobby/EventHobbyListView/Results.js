...
const Results = ({ className, render, setRender, ...rest }) => {
  ...
  const handleSubmit = (e, id) => {
    ...
    try {
      ...
      fetch(process.env.REACT_APP_URL + `admin/event_hobby/${info.id}`, {
        method: 'post',
        body: formData,
        headers: {
    ...
    try {
      fetch(
        process.env.REACT_APP_URL +
          `admin/events_hobbies/${limit}?page=${page + 1}`,
        {
          method: 'get',
          headers: {
            Accept: 'application/json',
  ...
  useEffect(() => {
    try {
      fetch(
        process.env.REACT_APP_URL + `admin/events_hobbies/${limit}?page=1`,
        {
          method: 'get',
          headers: {
            Accept: 'application/json',
  ...
  useEffect(() => {
    try {
      fetch(process.env.REACT_APP_URL + `admin/events/100000`, {
        method: 'get',
        headers: {
          Accept: 'application/json',
          Authorization: 'Bearer ' + tokenAdmin
  ...
  const handleDeleteOne = (e, id) => {
    ...
    if (window.confirm('Are you sure you want to delete it?'))
      try {
        fetch(process.env.REACT_APP_URL + `admin/event_hobby/${id}`, {
          method: 'delete',
          // body: formData,
          headers: {
            Accept: 'application/json',
                ...
                customers.slice(0, limit).map((customer, index) => {
                  return (
                    ...
                    >
                      ...
                      {customer.hobby && customer.hobby.name && (
                        <TableCell>{customer.hobby.name}</TableCell>
                      )}

                      {customer.event && customer.event.name && (
                        <TableCell>{customer.event.name}</TableCell>
                      )}
                      {customer.event &&
                        customer.event.user &&
                        customer.event.user.name && (
                          <TableCell>{customer.event.user.name}</TableCell>
                        )}
